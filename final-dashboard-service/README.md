...
`local_api.py` runs on the user's computer and binds to `127.0.0.1:8766`. Start the dashboard on port 8765 as shown in the root README. The bridge also accepts the usual Vite development origin on port 5173. It requires Pillow and pypdfium2, plus OpenSSH. Configure `OCR_BENCH_DGX_HOST`, `OCR_BENCH_DGX_USER`, `OCR_BENCH_DGX_ROOT`, and `OCR_BENCH_SSH_KEY_PATH` in the process environment before launching it. `.env.example` at the repository root documents optional values; the bridge does not load that file automatically.
